---
sidebar_position: 8
---

# Tenant Page

Welcome to the Tenant Page. You can see All the Tenant details Here. There are 5 Main Sections about using the Tenant Page:
- Profile
- Previous Messages
- Extraction
- Contacts
- Lease Documents

## Profile

The Profile Allows you to:

Edit the Tenant

![Docs Version Dropdown](./img/property2.png)

Archive the Tenant

![Docs Version Dropdown](./img/property3.png)

View the Unit the Tenant is in. If the Tenant Has Multiple Units, You can view other units the tenant occupies by clicking "Select Unit".

![Docs Version Dropdown](./img/tenant1.png)

Create a New Chat for the Tenant.

![Docs Version Dropdown](./img/tenant2.png)


## Previous Messages

You can view Previous Messages for the Tenant on the Previous Messages Panel. Clicking on a Message will take you to the [Chat Page](./chatPage.md).

![Docs Version Dropdown](./img/tenant3.png)

## Extraction
The Tenants will have terms that are extracted from the lease documents provided! View [Extraction](./Extraction.md) for more details!

## Contacts
Contacts are people who you contact for information about the tenant or unit they are leasing. You can add building contacts, accountants, lawyers, the tenant Manger, and anyone you email about this property.

The reason contacts are important is Email Integration. (Currently in Development) When Chatting with the Tenant, you can easily integrate your email so that when you ask a tenant Question, it will search your Inbox for Emails with the contacts of the tenant. This will give the AI additional context about the tenant you are asking about.

So add contacts of anybody's email can provide context to the questions!

![Docs Version Dropdown](./img/tenant4.png)

## Lease Documents
At the Bottom of the Tenant Page, you can see all of the lease documents that are uploaded for that tenant. If you are unsure wether a document is missing from a tenant, you can check the bottom of the Tenant Page.

Also sometimes there are errors when uploading documents. The bottom of the page will let you know if there was an error with the document, and it will allow you to reload that document right away

You can also click on the documents to view the leases right from lease link. So if you need a quick place to read a document, Lease Link has it

![Docs Version Dropdown](./img/tenant5.png)
