---
sidebar_position: 2
---

# General Navigation

Navigating Lease Link is fairly Simple. There are 5 buttons that will take you directly to pages on the Top Navigation Bar.

![Docs Version Dropdown](./img/topNav.png)

## Five Buttons

- Lease Link Logo -> [Dashboard](./Dashboard.md)

![Docs Version Dropdown](./img/logo.png)

- Chat Logo  -> [Chat Page](./chatPage.md)

![Docs Version Dropdown](./img/chatLogo.png)
- Upload Logo  -> [Upload Leases](./Upload.md)

![Docs Version Dropdown](./img/UploadLogo.png)
- Create Person Logo -> [Create Person](./Person.md)

 ![Docs Version Dropdown](./img/CreatePersonLogo.png)
- Create Property/Unit Logo  -> [Create Property/Unit](./facility.md)

![Docs Version Dropdown](./img/createPropertyLogo.png)

## Search Bar
The Search Bar will allow you to search for [Owners](./owner.md), [Properties](./property.md), [Tenants](./tenant.md), and [Units](./unit.md).

Simply Type in the Search Bar for the owner, property, tenant, or unit that you want to find, and it will find them. Once click it will take you to that entities page.

![Docs Version Dropdown](./img/appSearch.png)

## Profile Image
On the Right hand side of the screen will be a Profile Image or a person icon.

If you hover over the icon (or Profile Image) it will show 3 more pages:
- Profile
- Settings
- Sign Out

![Docs Version Dropdown](./img/profileNavigation.png)